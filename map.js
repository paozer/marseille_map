    ...
    for (m of markers) {
        if (m.name === marker) {
            if (image_or_text === "text")
                ...
                div.innerHTML = '<button onclick="switchPopupContent(\'' + m.name + '\', \'text\')">Mitlesen</button>' + '<p><a href="' + m.img_src + '" target="_blank" rel="noopener noreferrer">' + '<img src="' + m.img + '"' + '>' + '</a></p>';
            break;
        }
    }
}
